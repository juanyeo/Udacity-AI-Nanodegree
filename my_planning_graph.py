...
class ActionLayer(BaseActionLayer):
    ...
    def _interference(self, actionA, actionB):
        ...
        # if (effectsA == ~preconditionsB) or (effectsB == ~preconditionsA):
        for effectA in effectsA:
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        preconditionsA = self.parents[actionA]
        preconditionsB = self.parents[actionB]
        # if (preconditionsA == ~preconditionsB):
        for preconditionA in preconditionsA:
            for preconditionB in preconditionsB:
                if layer.is_mutex(preconditionA, preconditionB):
                    return True
        return False
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        # TODO: implement this function
        layer = self.parent_layer
        waysA = self.parents[literalA]
        waysB = self.parents[literalB]
        for wayA in waysA:
            for wayB in waysB:
                if not (layer.is_mutex(wayA, wayB)):
                    return False
        return True
...
class PlanningGraph:
    ...
    def h_maxlevel(self):
        ...
        self.fill()
        for goal in self.goal:
            for i in range(0, len(self.literal_layers)):
                if goal in self.literal_layers[i]:
                    costs.append(i)
            #costs.append(int(self.levelcost(goal)))
        return max(costs)
        ...
        for layer in self.literal_layers:
            ...
            for goalA in self.goal:
                for goalB in self.goal:
                    # if (wayA.expr == '~' + wayB.expr) or (wayB.expr == '~' + wayA.expr)
